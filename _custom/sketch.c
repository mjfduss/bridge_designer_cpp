...
void setup_image(IMAGE *image, UNSIGNED width, UNSIGNED height, RGB_TRIPLE *color)
{
	...
	clear_image(image);
	image->data = malloc(width * height * 2050);
...
#define decy i += w
	...
	if (dx > 0)
		if (dy > 0)
			if (dx > dy)
				bres_loop(incx, e += dy, e + e >= dx, incy, e -= dx) else bres_loop(incy, e += dx, e + e >= dy, incx, e -= dy) else if (dx + dy > 0)
					bres_loop(incx, e -= dy, e + e >= dx, decy, e -= dx) else bres_loop(decy, e -= dx, e + e <= dy, incx, e -= dy) else if (dy > 0) if (dx + dy < 0)
						bres_loop(decx, e -= dy, e + e <= dx, incy, e -= dx) else bres_loop(incy, e -= dx, e + e >= dy, decx, e -= dy) else if (dx < dy)
							bres_loop(decx, e += dy, e + e <= dx, decy, e -= dx) else bres_loop(decy, e += dx, e + e <= dy, decx, e -= dy)
}
...
#define chop(Code, X, Y)                                          \
	...
	{                                                             \
		...
		Code = clip_code(X, Y, x_left, y_top, x_right, y_bottom); \
	} while (0)
